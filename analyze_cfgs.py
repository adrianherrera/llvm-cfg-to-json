...
def get_num_indirect_calls(cfg: nx.DiGraph) -> int:
    """Determine the number of indirect function calls in the CFG."""
    return sum(data for _, data in cfg.nodes(data='indirect_calls') if data)
...
def main():
    ...
    for cfg_path in args.cfg:
        logging.info('Parsing %s...', cfg_path)
    ...
    for mod_data in modules:
        ...
        for func_data in mod_data['functions']:
            ...
            if func in blacklist:
                ...
                continue
            logging.debug('Processing %s (in %s)', func, module)
            ...
            for edge in edges:
                ...
                cfg.add_edge(src_bb, dst_bb, type='intra')
            ...
            for call in calls:
                ...
                dst_bb = create_cfg_node(callee_mod, callee_func,
                                         callee_data['entry'])
                cfg.add_edge(src_bb, dst_bb, type='call')
            ...
            # Count indirect calls and assign them to the CFG nodes that make
            # them
            indirect_call_count = Counter(func_data.get('indirect_calls', []))
            for bb in indirect_call_count:
                node = create_cfg_node(module, func, bb)
                cfg.nodes[node]['indirect_calls'] = indirect_call_count[bb]
    ...
    print('num. indirect edges -> %d' % get_num_indirect_calls(cfg))
